...
export default function WorldPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && data) {
        setCompleted(new Set(data.map((d: any) => d.lesson_id)));
      }
  ...
  return (
    <main className="min-h-screen">
      <header className="flex items-center justify-between px-6 md:px-12 py-5">
        ...
        {activeKid && (
          <Companion character={activeKid.avatar_key as any} size={50} />
        )}
      </header>

      <section className="px-6 md:px-12 py-8 max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <div className="text-7xl mb-4 inline-block animate-bounce-soft">
            {world.icon}
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {lessons.map((lesson, idx) => {
              ...
              const prevCompleted =
                idx === 0 || completed.has(lessons[idx - 1].id);
              ...
              return (
                <button
                  ...
                  disabled={locked}
                  onClick={() =>
                    !locked && router.push(`/app/lesson/${lesson.id}`)
                  }
                  className={clsx(
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-ink-700 text-sm mt-1">
                      {lesson.estimated_minutes} min • {lesson.steps.length}{' '}
                      steps
                    </p>
                  </div>
                  {!locked && (
