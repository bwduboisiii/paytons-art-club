...
import Companion, { ALL_COMPANIONS } from '@/components/Companion';
        ...
        <div className="card-cozy p-8 md:p-12">
          {step === 'name' && (
            <>
              ...
              <h1 className="heading-1 text-center mb-3">Let's set up your artist!</h1>
              ...
              <input
                ...
                placeholder="Kid's name" maxLength={30} autoFocus
              />
              <div className="flex justify-end mt-8">
                <Button variant="primary" size="lg" disabled={!name.trim()} onClick={() => setStep('age')}>
                  Next →
                </Button>
              </div>
            </>
          ...
          {step === 'age' && (
            <>
              ...
              <p className="text-center text-ink-700 mb-8 text-lg">We use this to pick just-right lessons.</p>
              <div className="grid grid-cols-5 md:grid-cols-10 gap-3">
                {Array.from({ length: 10 }).map((_, i) => {
                  const a = i + 3;
                  return (
                    <button
                      key={a} onClick={() => setAge(a)}
                      className={clsx(
          ...
          {step === 'companion' && (
            <>
              ...
              <p className="text-center text-ink-700 mb-6 text-lg">
                You can always change this later.
              </p>
              <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
                {ALL_COMPANIONS.map((c) => (
                  <button
                    key={c.key} onClick={() => setCompanion(c.key)}
                    className={clsx(
                      'card-cozy p-2 transition-all',
                      companion === c.key
                        ? 'scale-105 border-coral-400 shadow-chunky'
                        : 'hover:scale-105'
                    )}
                  >
                    <Companion character={c.key} mood={companion === c.key ? 'happy' : 'idle'} size={70} />
                    <p className="font-display font-bold text-center mt-1 text-xs">{c.name}</p>
                  </button>
                ))}
              </div>
              {error && (
                <p className="text-coral-600 mt-4 text-center bg-coral-300/30 border-2 border-coral-400 rounded-2xl p-3">{error}</p>
              )}
              <div className="flex justify-between mt-8">
