...
export default function OnboardingPage() {
  ...
  const [name, setName] = useState('Payton');
  ...
  async function finish() {
    ...
    const { data, error } = await supabase
      .from('kids')
      .insert({ parent_id: user.id, name, age, avatar_key: companion })
      .select()
      .single();
    if (error) {
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {COMPANIONS.map(c => (
                  ...
                  >
                    <Companion character={c.key} mood={companion === c.key ? 'happy' : 'idle'} size={100} />
                    <p className="font-display font-bold text-center mt-2">{c.name}</p>
                  </button>
                ))}
              </div>
              {error && <p className="text-coral-600 mt-4 text-center">{error}</p>}
              <div className="flex justify-between mt-8">
