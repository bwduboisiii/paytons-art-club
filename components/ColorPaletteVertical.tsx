...
interface Props {
  colors?: string[]; // kept for API compat
  selected: string;
...
export default function ColorPaletteVertical({ selected, onChange, className }: Props) {
  const [pickerOpen, setPickerOpen] = useState(false);

  return (
    <div className={clsx('flex flex-col gap-2 h-full', className)}>
      <div className="flex flex-col gap-1.5 items-center overflow-y-auto no-scrollbar flex-1 py-1">
        {FULL_PALETTE.map((c) => {
          ...
          return (
            <button
              ...
              onClick={() => onChange(c)}
              className={clsx(
                'shrink-0 w-10 h-10 rounded-full transition-all duration-150 border-[3px]',
        ...
        <button
          ...
          className="shrink-0 w-10 h-10 rounded-full border-[3px] border-white shadow-float hover:scale-110 transition-transform"
          style={{
      ...
      {pickerOpen && (
        <div className="flex flex-col items-center gap-1 bg-cream-100 rounded-2xl p-2">
          <input
            type="color"
            value={selected}
